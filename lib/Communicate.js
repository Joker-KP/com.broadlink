...
class Communicate {
	...
	async sendto(packet, ipaddress, port, timeout) {
		...
		if (this.dgramSocket === undefined) {
			...
			this.dgramSocket.on('message', (msg, rinfo) => {
				...
				this._utils.debugLog('<== Communicate.sendto - got message with size=' + rsp['size']);
		...
		return new Promise(function (resolve, reject) {
			...
			this._utils.debugLog(`==> Communicate.sendto - send packet to ${ipaddress}:${port}. Data: ${packet}`)
			this.tm = setTimeout(function () {
				clearTimeout(this.tm);
				this.tm = undefined;
				this._utils.debugLog('**> dgramSocket timeout');
				if (this.dgramSocket !== undefined) {
					this.dgramSocket.close();
					this.dgramSocket = undefined;
				}
	...
	async send_packet(command, version, payload) {
		this._utils.debugLog('->send_packet: payload=' + this._utils.asHex(payload));
		...
		// send packet
		let response = await this.sendto(packet, this.ipAddress, 80, 20)
		this._utils.debugLog('->send_packet: resp=' + this._utils.asHex(response.data))

		if (response) {
			this._utils.debugLog('got response:' + this._utils.asHex(response.data))
			response.error = response.data[0x22] | (response.data[0x23] << 8)
			this._utils.debugLog(' error = ' + response.error)
			if (response.error == 0) {
				...
				this._utils.debugLog('<-send_packet: resp.payload=' + this._utils.asHex(response.cmd) + ":" + this._utils.asHex(response.payload))
			}
		}
		else {
			response.error = -1
	...
	async auth() {
		...
		this._utils.debugLog('<== auth: auth data :' + JSON.stringify(authData));
	...
	async _check_data(payload) {
		...
		return new Promise(function (resolve, reject) {
			let tm = setInterval(function () {
				this._utils.debugLog('_check_data - repeat=' + this.checkRepeatCount)
					...
					.then(
						function (response) {
							if (response.error == 0) {
								clearInterval(tm)
								tm = null
								this._utils.debugLog('resp = ' + this._utils.asHex(response.payload))
						...
						rejection => {
							this._utils.debugLog('**> _check_data - reject: ' + rejection)
							clearInterval(tm);
							tm = null;
							reject('aborted');
						})
					.catch(err => {
						this._utils.debugLog('**> _check_data - catch: ' + err)
						clearInterval(tm);
						tm = null;
						reject(err);
					})
