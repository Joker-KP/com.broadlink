...
const MAX_LOG_LENGTH = 250; // Maximum length of the logged response
...
class Communicate {
  ...
  async sendto(packet, ipaddress, port, timeout) {
    ...
    return new Promise(
      function (resolve, reject) {
        ...
        this._utils.debugLog(this, `==> Communicate.sendto - send packet to ${ipaddress}:${port}. Data: ${packet}`);
        this.tm = setTimeout(
          function () {
            clearTimeout(this.tm);
            this.tm = undefined;
  ...
  async send_packet(command, version, payload) {
    ...
    this._utils.debugLog(this, "->send_packet: (truncated) resp=" + (this._utils.asHex(response.data).length > MAX_LOG_LENGTH ? this._utils.asHex(response.data).substring(0, MAX_LOG_LENGTH) + '...' : this._utils.asHex(response.data)));

    if (response) {
      this._utils.debugLog(this, "(truncated) got response:" + 
    (this._utils.asHex(response.data).length > MAX_LOG_LENGTH 
        ? this._utils.asHex(response.data).substring(0, MAX_LOG_LENGTH) + '...' 
        : this._utils.asHex(response.data))
);
      response.error = response.data[0x22] | (response.data[0x23] << 8);
      this._utils.debugLog(this, " error = " + response.error);
      if (response.error == 0) {
        ...
        this._utils.debugLog(
          this,
          "<-send_packet: resp.payload (truncated)=" + 
          this.ipAddress + ":" + 
              ...
              .then(
                function (response) {
                  if (response.error == 0) {
                    clearInterval(tm);
                    tm = null;
                    this._utils.debugLog(this, "resp = " + this._utils.asHex(response.payload));
  ...
  async check_IR_data() {
    ...
    if (this.deviceType == 0x5f36) {
      ...
      payload[1] = 0;
      payload[2] = 4;
      this._utils.debugLog(this, `Modified payload, device Red Bean :`, this.deviceType);
    } else {
      this._utils.debugLog(this, `Classic RM3 device :`, this.deviceType);
      payload = new Uint8Array(16);
      payload[0] = 4;
    }
