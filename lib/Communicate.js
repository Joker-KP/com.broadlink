...
class Communicate {
  ...
  async send_packet(command, version, payload) {
    ...
    if (response) {
      ...
      this._utils.debugLog(this, " We got error = " + response.error);
      response.error = -1;
    }
  ...
  async checkTemperature_rm4pro() {
    this._utils.debugLog(this, `>>> Checking temperature <<<`);
    var payload = new Uint8Array(16);
    payload[2] = 0x24;
    await this.send_packet(0x6a, false, payload);
  }

  async checkHumidity_rm4pro() {
    this._utils.debugLog(this, `>>> Checking humidity <<<`);
    var payload = new Uint8Array(16);
    payload[2] = 0x24;
    await this.send_packet(0x6a, false, payload);
  }
