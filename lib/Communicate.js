...
class Communicate {
  ...
  async send_packet(command, version, payload) {
    ...
    this._utils.debugLog(this,"->send_packet - response= " +(this._utils.asHex(response.data).length > MAX_LOG_LENGTH
  ...
  async enter_learning() {
    this._utils.debugLog(this, ">> Entering IR learning <<");
  ...
  async enter_learning_red() {
    this._utils.debugLog(this, ">> Entering IR learning, Red Bean or RM4 <<");
    var payload = new Uint8Array(16);
    payload[0] = 4;
    payload[1] = 0;
    payload[2] = 3;
  ...
  async check_IR_data() {
    var payload;
    // removing this for 0x5f36 as we have the separate method anyway and seems it did not work for 0x5f36 anyway
    /*if (this.deviceType === 0x5f36) {
      // RM Mini 3 Red Bean device type
      payload = new Uint8Array(16);
      payload[1] = 0;
      payload[2] = 4;
      this._utils.debugLog(this, `Modified payload, device Red Bean or RM4: `, this.deviceType);
    } else {*/
