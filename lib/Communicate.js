...
class Communicate {
  ...
  async send_packet(command, version, payload) {
    ...
    this._utils.debugLog(this, "->send_packet: resp=" + this._utils.asHex(response.data));

    if (response) {
      this._utils.debugLog(this, "got response:" + this._utils.asHex(response.data));
      ...
      if (response.error == 0) {
        ...
        this._utils.debugLog(
          this,
          "<-send_packet: resp.payload=" + this._utils.asHex(response.cmd) + ":" + this._utils.asHex(response.payload)
        );
      }
    } else {
      response.error = -1;
  ...
  async auth() {
    ...
    this._utils.debugLog(this, "<== auth: auth data :" + JSON.stringify(authData));
