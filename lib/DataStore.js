  ...
  readCommands() {
    return new Promise((resolve, reject) => {
      ...
      fs.access(fileName, fs.constants.F_OK | fs.constants.W_OK | fs.constants.R_OK, (err) => {
        if (err) {
          this.debugLog(null, "**> file does not exist or no access: " + err);
          return resolve();
        } else {
          fs.readFile(fileName, "utf8", (err, data) => {
            if (err) {
              this.debugLog(null, "**> readFile error: " + err);
              return reject(err);
            } else {
              try {
                this.dataArray = [];
                ...
                for (let i = 0; i < arr.length; i++) {
                  ...
                  //this.debugLog(null, `**> Command ${i} added:`, elem); // Debug log for each command added
                }
                return resolve();
              } catch (err) {
                this.debugLog(null, "**> parse failed: " + err);
                return reject(err);
              }
            }
          });
