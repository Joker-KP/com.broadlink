...
class DataStore {
  ...
  storeCommands() {
    ...
    console.log(`Storing commands to file: ${fileName}`);

    fs.writeFile(fileName, data, (err) => {
      if (err) {
        console.log(`**> Storing dataArray failed: ${err}`);
      } else {
  ...
  readCommands() {
    return new Promise((resolve, reject) => {
      ...
      fs.access(fileName, fs.constants.F_OK | fs.constants.W_OK | fs.constants.R_OK, (err) => {
        if (err) {
          console.log("**> file does not exist or no access: " + err);
          return resolve();
        } else {
            ...
            if (err) {
              ...
              try {
                ...
                for (let i = 0; i < arr.length; i++) {
                  ...
                  //console.log(`**> Command ${i} added:`, elem); // Debug log for each command added
                }
                return resolve();
              } catch (err) {
